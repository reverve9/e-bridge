...
import { Save, Plus, Trash2, Edit2, X, Link, ExternalLink, Sparkles, Loader2, Camera, Newspaper, Pin } from 'lucide-react';
...
interface CandidateFeedsTabProps {
  candidateId: string;
  onUpdate?: () => void;
}
...
export default function CandidateFeedsTab({ candidateId, onUpdate }: CandidateFeedsTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-gray-900">소식 관리</h3>
          <p className="text-sm text-gray-500 mt-1">활동, 뉴스, 공지 등 후보자 소식을 관리합니다.</p>
        </div>
        <button
      ...
      {feeds.length === 0 ? (
        ...
        <div className="space-y-3">
          {feeds.map((feed) => (
            <div key={feed.id} className="bg-gray-50 rounded-xl p-4">
              ...
              <h4 className="font-medium text-gray-900 mb-1">{feed.title}</h4>
              {feed.summary && (
      ...
      {showModal && (
        ...
        >
          <div
            ...
            className="bg-white w-full max-w-lg rounded-2xl max-h-[85vh] overflow-hidden flex flex-col"
          >
            {/* 모달 헤더 */}
